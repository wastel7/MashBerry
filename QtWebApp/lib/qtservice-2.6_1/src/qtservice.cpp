...
int QtServiceBase::exec()
{
    if (d_ptr->args.size() > 1) {
        ...
        if (a == QLatin1String("-i") || a == QLatin1String("-install")) {
            ...
            return 0;
        } else if (a == QLatin1String("-s") || a == QLatin1String("-status")) {
            printf("The service\n"
            ...
            printf("\n%s -[i|u|e|s|v|h]\n"
                   ...
                   "\t-e(xec)\t\t: Start as regular application. Useful for debugging.\n"
                   "\t-t(erminate)\t: Stop the service.\n"                   
                   "\t-s(tatus)\t: Print status information.\n"
                   "\t-h(elp)   \t: Show this help\n"
                   "\tNo arguments\t: Start the service.\n",
                   d_ptr->args.at(0).toLatin1().constData());
            // Removed help of unused option: "\t-c(ommand) num\t: Send command code num to the service.\n"
            return 0;
        }
    }
...
#endif
    if (!d_ptr->start()) {
        fprintf(stderr, "The service %s could not start. Use option -h for help.\n", serviceName().toLatin1().constData());
        return -4;
    }
    return 0;
}
